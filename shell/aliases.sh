...
tbd() {
    local cmd
    if [[ $# -eq 0 ]]; then
        cmd="tensorboard serve --reload_interval 30 --logdir ."
    elif [[ $# -eq 1 ]]; then
        cmd="tensorboard serve --reload_interval 30 --logdir $1"; shift
    elif [[ $# -eq 2 ]]; then
        cmd="tensorboard serve --reload_interval 30 --logdir $1 --port $2"; shift; shift
    else
